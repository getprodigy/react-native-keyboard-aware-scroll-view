...
const KeyboardAwareMixin = {
  ...
  componentDidMount: function () {
    ...
    if (Platform.OS === 'ios') {
      ...
      this.id = `keyboardMixin-${Math.random()}`; // Create a random id for current keyboard view component
      Events.on('scrollTo', this.id, (currentlyFocusedField = TextInput.State.currentlyFocusedField()) => {
        const responder = this.getScrollResponder();
        if (!currentlyFocusedField || !responder) {
          return
        }
